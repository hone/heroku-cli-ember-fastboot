...
function deployCommand(context, heroku) {
  let emberBuild = spawn('ember', ['build', '--environment', 'production']);
  let fastBootBuild = spawn('ember', ['fastboot:build', '--environment', 'production']);

  emberBuild.stdout.on('data', (data) => {
    console.log(`ember:    ${data.toString().trim()}`);
  });

  emberBuild.stderr.on('data', (data) => {
    console.error(`ember:    ${data.toString().trim()}`);
  });
  ...
  emberBuild.on('close', (emberCode) => {
    fastBootBuild.on('close', (fastBootCode) => {
      if(emberCode == 0 && fastBootCode == 0) {
        ...
        console.error(`Couldn't build assets, ${emberCode}, ${fastBootCode}`);
      }
    })
  })
}
